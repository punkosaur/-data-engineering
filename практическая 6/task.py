...
file_name = "dataset/Russia_Real_Estate_2021.csv"
...
def figure3(read_and_optimized):
  read_and_optimized['year'] = pd.to_datetime(read_and_optimized['date']).dt.year
  avg_price_by_year = read_and_optimized.groupby('year')['price'].mean()
  plt.figure(figsize=(10, 6))
  plt.plot(avg_price_by_year.index, avg_price_by_year.values)
  plt.title('Средняя цена квартиры по годам')
  plt.xlabel('Год')
...
def figure4(read_and_optimized):
  building_counts = read_and_optimized['building_type'].value_counts()
  plt.figure(figsize=(8, 8))
  plt.pie(building_counts, labels=building_counts.index, autopct='%1.1f%%', startangle=140)
  plt.title('Доля квартир по типу здания')
  plt.axis('equal')  # Equal aspect ratio ensures that pie is drawn as a circle.
  plt.show()

def figure5(read_and_optimized):
  plt.figure(figsize=(10, 6))
  sns.scatterplot(x='geo_lon', y='geo_lat', data=read_and_optimized, alpha=0.5) # альфа уменьшает насыщенность точек для лучшей видимости
  plt.title('Зависимость geo_lat от geo_lon')
  plt.xlabel('geo_lon')
  plt.ylabel('geo_lat')
  plt.show()
...
def figure6(read_and_optimized):
  plt.figure(figsize=(10,6))
  sns.boxplot(x='rooms', y='price', data=read_and_optimized)
